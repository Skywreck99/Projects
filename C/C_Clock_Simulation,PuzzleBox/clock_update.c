...
// penalized for complexity.
  ...
  if ((tod.hours & (0b1 << 31)) || (tod.minutes & (0b1 << 31)) || (tod.seconds & (0b1 << 31)) || (tod.ispm & (0b1 << 31))) {
    ...
    *display = masks[min_ones] | masks[10];
    ...
    *display = (masks[hour_ones] << 14) | *display;
    ...
    if (tod.hours / 10 == 0) *display = 0b0000000 << 21 | *display; else *display = (masks[hour_tens] << 21) | *display;
...
// memory only.
  ...
  if (result) {
    ...
    set_display_bits_from_tod(tod, &CLOCK_DISPLAY_PORT);
    return 0;
  }
}
