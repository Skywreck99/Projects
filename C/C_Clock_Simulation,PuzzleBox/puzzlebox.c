...
int main(int argc, char *argv[]){
  ...
  printf("PHASE 8: You're doing great, now try phase eight!\n");
  phase08();
  score+=points[8];
...
void phase06(){
  ...
  int *iact = (int *) actual;
  iact[munge1] = 0; // How does this slice the string???
  iact[munge2] = 0;
...
void phase10(){
  ...
  char *accept = "Determination!";
  if(strcmp(ans,accept)==0){
    printf("         '%s' : you definitely got it. Congrads!\n",ans);
    return;
  }

  char msg[BUFSIZE] = {};
  snprintf(msg, BUFSIZE,
           "So close, but '%s' is not the quality you exhibited after all this trouble.\n",
           ans);
